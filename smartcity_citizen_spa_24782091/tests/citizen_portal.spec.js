...
//   Opsi 1 (Live Server): 'http://127.0.0.1:5500/index.html'
//   Opsi 2 (http-server):  'http://localhost:8080/index.html'
//   Opsi 3 (file://):      'file:///C:/Users/.../smartcity_citizen_spa/index.html'
const SPA_URL = 'https://iet-polinela.github.io/project-2026-RagilAfda/';
 ...
 *
 * Menggunakan wildcard API global, fungsi ini akan mencegat request ke domain apapun
...
/**
 * mockCurrentCitizen - Mock endpoint /api/me/ agar token dummy dianggap
 * sebagai warga biasa yang sudah login.
 *
 * @param {import('@playwright/test').Page} page - Objek halaman Playwright
 */
async function mockCurrentCitizen(page) {
...
test.describe('Modul 1: Otorisasi & Sesi (AUTH-04, AUTH-05, AUTH-06)', () => {
    ...
    test('AUTH-05: Token kadaluarsa → interceptor menangani 401 dan redirect ke #login', async ({ page }) => {
        ...
        //
        // Pola URL API akan mencocokkan semua request
        // ke endpoint report API (termasuk query parameters).
        //
        // -------------------------------------------------------------------

        // Hapus interceptor URL sebelumnya agar mock 401 ini menjadi prioritas
...
test.describe('Modul 5: Interaktivitas UI (UI-01 through UI-06)', () => {
    ...
    //   - Dashboard mengambil data dari /dashboard/data/ via fetch()
    ...
    test('UI-01: Chart.js di Dashboard Admin ter-render dengan benar', async ({ page }) => {
        ...
        // Kita perlu menunggu:
        //   1. Fetch ke /dashboard/data/ selesai
        ...
        // -------------------------------------------------------------------
        // Dashboard juga menampilkan 2 tabel: latestReportedTable dan latestResolvedTable
    ...
    test('UI-02: Live Search pada daftar laporan admin berfungsi', async ({ page }) => {
        ...
        console.log(`[UI-02] Hasil pencarian "${searchKeyword}": ${responseData.length || 0} item`);
        ...
        if (Array.isArray(responseData) && responseData.length > 0) {
            ...
            expect(filteredRowCount).toBe(responseData.length);
        }
    ...
    test('UI-03: Pagination Feed Kota — maks 10 kartu, kontrol pagination muncul', async ({ page }) => {
        ...
        // -------------------------------------------------------------------
        // LANGKAH 3: Navigasi ke halaman laporan
        // -------------------------------------------------------------------
        await page.goto(`${SPA_URL}#reports`);
        await page.waitForSelector('#listContainer', { state: 'visible', timeout: 10000 });

        // -------------------------------------------------------------------
        // LANGKAH 4: Klik tab "Feed Kota (Publik)"
        // -------------------------------------------------------------------
        // Tab ini ada di renderReportsPage() sebagai data-report-tab="feed"
        const tabFeedKota = page.locator('[data-report-tab="feed"]');
        ...
        // -------------------------------------------------------------------
        // Setiap laporan dirender sebagai <div class="col-12"> di dalam #listContainer
    ...
    //
    // SKENARIO:
    //   Login ke SPA, navigasi ke #dashboard, klik tombol create report,
    ...
    //     tombol data-create-report membuka modal Bootstrap
    ...
    test('UI-04: Klik tombol Buat Laporan → modal #reportModal muncul', async ({ page }) => {
        ...
        const btnBukaModal = page.locator('[data-create-report]');
    ...
    //   3. Badge count Draf di #draftCount terupdate
    //
    // REFERENSI KODE:
    ...
    test('UI-05: Isi form dan simpan draft → modal tutup, notifikasi muncul', async ({ page }) => {
        ...
        await page.route('**/api/reports/**', async (route) => {
            ...
            if (method === 'POST') {
                // -----------------------------------------------------------
                // Mock untuk POST /api/reports/ (membuat laporan baru)
                // -----------------------------------------------------------
                draftSubmitted = true;
            ...
            } else if (method === 'GET' && url.includes('page_size=100')) {
                // -----------------------------------------------------------
                // Mock untuk GET /api/reports/?tab=my_reports&page_size=100
        ...
        // -------------------------------------------------------------------
        await page.goto(`${SPA_URL}#dashboard`);
        await page.waitForSelector('[data-create-report]', { state: 'visible', timeout: 10000 });

        // Klik tombol buka modal
        await page.locator('[data-create-report]').click();
        ...
        await page.waitForTimeout(2000);
        await page.goto(`${SPA_URL}#reports`);
        await page.waitForSelector('#draftCount', { state: 'visible', timeout: 10000 });

        const draftBadge = page.locator('#draftCount');
